...
public class ZipUtility {
	...
	private void addFolder(ZipOutputStream out, String basePath, File dir) throws IOException {
		...
		//非空目錄,建立檔案的時候就會建立
		//空目錄的話,就必須手動加入zip檔
		//若用寫檔案的方式用FileInputStream讀目錄,是會丟出Exception的
		if(files.length == 0) {
			//ZipEntry如何分辨目錄,是看它的name是否是SEPARATOR結尾
			//可以看ZipEnry的isDirectory的java doc說明
	...
	public void unzip(File zipFile, String basePath) throws ZipException, IOException {
        ...
        try {
	        ...
	        while(e.hasMoreElements()) {
	           ...
	           //檔案不存在要先建檔再寫檔,否則FileOutputStream會拋出exception
	           if(!file.exists()) {
