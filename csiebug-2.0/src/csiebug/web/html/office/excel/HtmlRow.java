...
//	        	設定各樣屬性
	        ...
	        //order
	        HtmlColumn order = new HtmlColumn();
	        order.setRow(this);
	        order.setFieldname("order");
	        order.setStyle("display:none");
	        htmlBuilder.appendString(order.render("<input type=\"hidden\" id=\"(var.TableId)_(var.SerialNumber)_(var.ColumnIndex)\" name=\"(var.TableId)_(var.SerialNumber)_(var.ColumnIndex)\" value=\"(var.order)\"></input>"));
