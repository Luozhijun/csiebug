...
public class KeyValueAction extends BasicAction {
	private static final long serialVersionUID = 1L;

	public String main() throws Exception {
		//以下的if else處理各樣頁面的動作,可自行增加或修改
		if(getActFlag().startsWith("keyValue")) {
	...
	private void makeControl() throws UnsupportedEncodingException, NamingException {
		...
		try {
			...
			if(AssertUtility.isNotNullAndNotSpace(getFunctionId())) {
				...
				setRequestAttribute("FunctionName", getMessage("taglibdemo.KeyValueDoc.FunctionName"));
			}
		}
